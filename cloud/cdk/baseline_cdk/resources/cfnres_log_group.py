...
def create_layer_zip() -> str:
    ...
    shell(f'bash {cloud_dir}/scripts/aws-lambda-pip.sh'
          ...
          f'  -req "{this_dir}/cfnres/log_group/requirements.txt"')
...
def create_lambda_zip() -> str:
    ...
    with zipfile.ZipFile(lambda_zip, 'w', zipfile.ZIP_DEFLATED) as zip:
        zip.write(f'{this_dir}/cfnres/log_group/index.py', arcname='index.py')

    return lambda_zip


def create_lambda(stack: core.Stack) -> aws_lambda.CfnFunction:
    lambda_scope = core.Construct(stack, 'LogGroupLambda')
...
def CfnLogGroup(stack: core.Stack, scope: core.Construct, id: str, log_group_name: str, retention_in_days: int) -> aws_cloudformation.CfnCustomResource:
    lambda_function: aws_lambda.CfnFunction = cdk.find_resource(stack, 'LogGroupLambda/Function')
    ...
    custom_resource.add_override('Type', 'Custom::LogGroup')
    custom_resource.add_override('Properties.LogGroupName', log_group_name)
    custom_resource.add_override('Properties.RetentionInDays', retention_in_days)
